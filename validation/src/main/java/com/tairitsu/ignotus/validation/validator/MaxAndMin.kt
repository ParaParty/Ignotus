...
class ValidatorMax : AttributeValidatorInterface {
    ...
    override fun invoke(attribute: String, arg: Any?, value: Any?, fail: (String) -> Unit) {
        if (arg == null) {
            val s =
                lang("validator_error.unexpected_argument.null", mapOf("validator" to this::javaClass.name))
            log.warn(s)
            fail(s)
            return
        }
        ...
        try {
            when (value) {
                is Double -> {
                    ...
                    if (value > maxValue) fail(lang("validation.max.numeric",
                        mapOf("attribute" to ValidatorAttributesHelper.getAttributeFriendlyName(attribute),
                            "max" to maxValue)))
                }
                is Float -> {
                ...
                is Number -> {
                    val maxValue = ValidatorArgumentsHelper.parseToDouble(arg)
                    if (value.toDouble() > maxValue) fail(lang("validation.max.numeric",
                        mapOf("attribute" to ValidatorAttributesHelper.getAttributeFriendlyName(attribute),
                            "max" to maxValue)))
                }
                is String -> {
                    val maxValue = ValidatorArgumentsHelper.parseToInt(arg)
                    if (value.length > maxValue) fail(lang("validation.max.string",
                        mapOf("attribute" to ValidatorAttributesHelper.getAttributeFriendlyName(attribute),
                            "max" to maxValue)))
                }
                is Iterable<*> -> {
                    val maxValue = ValidatorArgumentsHelper.parseToInt(arg)
                    if (value.count() > maxValue) fail(lang("validation.max.array",
                        mapOf("attribute" to ValidatorAttributesHelper.getAttributeFriendlyName(attribute),
                            "max" to maxValue)))
                }
            ...
            fail(lang("validator_error.unexpected_argument.general", mapOf("validator" to this::javaClass.name)))
        }
    }
}
        ...
        try {
            when (value) {
                is Double -> {
                    ...
                    if (value < minValue) fail(lang("validation.min.numeric",
                        mapOf("attribute" to ValidatorAttributesHelper.getAttributeFriendlyName(attribute),
                            "min" to minValue)))
                }
                is Float -> {
                ...
                is Number -> {
                    val minValue = ValidatorArgumentsHelper.parseToDouble(arg)
                    if (value.toDouble() < minValue) fail(lang("validation.min.numeric",
                        mapOf("attribute" to ValidatorAttributesHelper.getAttributeFriendlyName(attribute),
                            "min" to minValue)))
                }
                is String -> {
                    val minValue = ValidatorArgumentsHelper.parseToInt(arg)
                    if (value.length < minValue) fail(lang("validation.min.string",
                        mapOf("attribute" to ValidatorAttributesHelper.getAttributeFriendlyName(attribute),
                            "min" to minValue)))
                }
                is Iterable<*> -> {
                    val minValue = ValidatorArgumentsHelper.parseToInt(arg)
                    if (value.count() < minValue) fail(lang("validation.min.array",
                        mapOf("attribute" to ValidatorAttributesHelper.getAttributeFriendlyName(attribute),
                            "min" to minValue)))
                }
                else -> log.warn("Validator ${this::javaClass.name} can not parse value of ${value::javaClass.name} type.")
